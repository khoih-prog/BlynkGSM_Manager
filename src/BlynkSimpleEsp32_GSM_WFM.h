...
#define BLYNK_CONNECT_TIMEOUT_MS      10000L
    ...
    {          
      ...
      {           
        if (BlynkGSM_ESP32_config.WiFi_Creds[i].wifi_pw && strlen(BlynkGSM_ESP32_config.WiFi_Creds[i].wifi_pw))
        {
#if ( BLYNK_WM_DEBUG > 2)
          BLYNK_LOG1(BLYNK_F("con2WF:begin"));
#endif
          WiFi.begin(BlynkGSM_ESP32_config.WiFi_Creds[i].wifi_ssid, BlynkGSM_ESP32_config.WiFi_Creds[i].wifi_pw);
        }
        else
        {
          WiFi.begin(BlynkGSM_ESP32_config.WiFi_Creds[i].wifi_ssid);
        }
