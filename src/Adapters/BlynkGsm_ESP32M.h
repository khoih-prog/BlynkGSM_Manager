...
#ifndef BlynkGsm_ESP32M
#define BlynkGsm_ESP32M
...
public:
  	...
  	void begin(TinyGsm& gsm, const char *iHostname = "")
    {
        ...
        if (getConfigData())
        {
          ...
          if (connectToGPRSNetwork(TIMEOUT_CONNECT_GPRS)) 
          {
  ...
  // Force range of user-defined TIMES_BEFORE_RESET between 2-100
    ...
    void run()
    {
      ...
      if ( !modem->isGprsConnected() || !connected() )
      {   
...
#define NO_CONFIG       "nothing"
    ...
    {     
      ...
      if (strncmp(BlynkGSM_ESP32_config.header, BOARD_TYPE, strlen(BOARD_TYPE)) != 0) 
      {
          ...
          strcpy(BlynkGSM_ESP32_config.gprsPin,          NO_CONFIG);
          ...
          return false;        
                ...
                !strncmp(BlynkGSM_ESP32_config.gprsPin,          NO_CONFIG, strlen(NO_CONFIG) )  ||
  ...
  #define EEPROM_START     0
    ...
    bool connectToGPRSNetwork(int timeout)    //(const char* apn, const char* user, const char* pass)
    {
